...
void Prevoyante::move(Bestiole &b, vector<Bestiole const *> const &seen_neighbors) {
    if (seen_neighbors.empty()) {
        return nullptr;
    }
    for (auto neighbor: seen_neighbors) {
        auto directionX = neighbor->getCoordinates().first - b.getCoordinates().first;
        auto directionY = neighbor->getCoordinates().second - b.getCoordinates().second;
